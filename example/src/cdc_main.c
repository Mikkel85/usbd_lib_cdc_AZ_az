...
static const USBD_API_T g_usbApi = { &hw_api, &core_api, 0, 0, 0, &cdc_api, 0,
		0x02221101, };
...
char* str_flip_case(char * string) {
	char *chr = string;
	while (*chr) {
		if (*chr >= 'a' && *chr <= 'z') {
...
int main(void) {
	...
	while (1) {
		...
		if ((vcom_connected() != 0) && (prompt == 0)) {
			...
			__WFI(); // Wait for interupt, so the endpoint buffer can be send.
		...
		if (prompt) {
			...
			//Upper to lower user function
			str_flip_case(g_rxBuff);
			if (rdCnt) {
